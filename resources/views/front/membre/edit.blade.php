                ...
                <form action="{{route('membre.update', $user->id)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="prenom" value="{{$user->prenom}}" class="form-control" id="prenom"
                                   placeholder="Titre de l'évènement">
                            @if($errors->has('prenom')) <span class="error bg-warning">{{$errors->first('prenom')}}</span>@endif
                        </div>

                        <div class="form-group">
                                    <label for="exampleFormControlInput1">E-mail</label>
                                    <input name="email" value="{{$user->email}}"type="text" class="form-control" id="exampleFormControlInput1" placeholder="email">
                                    @if($errors->has('email')) <span class="error bg-warning ">{{$errors->first('email')}}</span>@endif
                        </div>
                        ...
                        <div class="form-group">
                                    ...
                                    @if($errors->has('adresse')) <span class="error bg-warning ">{{$errors->first('adresse')}}</span>@endif
                            </div>
